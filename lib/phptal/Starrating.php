...
class PHPTAL_Php_Attribute_CHANGE_starrating extends ChangeTalAttribute 
{
	...
	public static function renderStarrating($params)
	{
		...
		echo '<li class="current-rating rating-'.$currentRating .' star">'. $ls->transFO('m.comment.frontoffice.current-star-rating', array('html'), array("rating" => round(floatval($params['value'])))) .'</li>';
		if ($params['displayOnly'] === false)
		{
			$format = array('attr');
			echo '<li class="star"><a href="#' . self::$idCounter . '" title="' . $ls->transFO('m.comment.frontoffice.star-rating-1', $format) .'" class="one-star'. ($currentValue == 1 ? ' clicked' : '') .'">1</a></li>';
			echo '<li class="star"><a href="#' . self::$idCounter . '" title="' . $ls->transFO('m.comment.frontoffice.star-rating-2', $format) .'" class="two-stars'. ($currentValue == 2 ? ' clicked' : '') .'">2</a></li>';
			echo '<li class="star"><a href="#' . self::$idCounter . '" title="' . $ls->transFO('m.comment.frontoffice.star-rating-3', $format) .'" class="three-stars'. ($currentValue == 3 ? ' clicked' : '') .'">3</a></li>';
			echo '<li class="star"><a href="#' . self::$idCounter . '" title="' . $ls->transFO('m.comment.frontoffice.star-rating-4', $format) .'" class="four-stars'. ($currentValue == 4 ? ' clicked' : '') .'">4</a></li>';
